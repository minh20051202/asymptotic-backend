...
3. Tổ chức hoặc người được phân quyền đăng ký AI Agent bên ngoài vào Gateway.
4. Gateway cấp API key cho Agent.
5. Tổ chức thiết lập policy, quota, budget hoặc hạn mức cho Agent.
6. AI Agent gửi request tới Gateway bằng API key do Asymptotic cấp.
7. Gateway xác thực API key và xác định Agent, tổ chức chịu chi phí.
8. Gateway kiểm tra policy, quota và budget.
9. Nếu hợp lệ, Gateway gọi AI Provider bằng provider credential nội bộ của Asymptotic.
10. Gateway nhận kết quả, ghi nhận usage/cost/trace và cập nhật trạng thái tài chính.
11. Gateway trả kết quả hoặc lỗi rõ ràng cho Agent.
...
- Phân bổ ngân sách cho developer, team hoặc Agent nếu cần mô hình hóa.
...
Quản trị viên tổ chức quản lý tổ chức, ví/ngân sách, developer, Agent, policy và báo cáo trong phạm vi tổ chức.

### A03 -- Lập trình viên

Lập trình viên thuộc một tổ chức, có thể đăng ký hoặc quản lý AI Agent theo quyền được cấp. Lập trình viên nhận API key để tích hợp Agent bên ngoài với Gateway.
...
- **Primary Actor:** Quản trị viên tổ chức.
- **Mục tiêu:** Phân bổ hoặc thu hồi ngân sách trong nội bộ tổ chức.
- **Must Have:** Có nếu mô hình ngân sách nhiều cấp được giữ trong báo cáo.

### UC04 -- Đăng ký và quản lý AI Agent

- **Primary Actor:** Quản trị viên tổ chức hoặc Lập trình viên.
- **Mục tiêu:** Đăng ký AI Agent bên ngoài vào Gateway, cập nhật trạng thái, gắn policy/budget và quản lý thông tin Agent.
- **Must Have:** Có.
- **Ghi chú:** Use case này cần tồn tại rõ ràng. Không nên chỉ ngầm hiểu qua quản lý API key hoặc quản lý hạn mức.
...
- **Primary Actor:** Quản trị viên tổ chức.
- **Mục tiêu:** Quản lý người dùng thuộc tổ chức, quyền truy cập và phạm vi ngân sách.
- **Must Have:** Có nếu báo cáo giữ mô hình developer trong tổ chức.
...
Hệ thống cho phép ghi nhận tổ chức sử dụng dịch vụ, số dư hoặc ngân sách khả dụng và các giao dịch nạp/ghi nhận chi phí tương ứng.

### FR02 -- Đăng ký và quản lý AI Agent

Hệ thống cho phép tổ chức hoặc người được phân quyền đăng ký AI Agent bên ngoài, gắn Agent với tổ chức, trạng thái hoạt động, policy/budget và lịch sử sử dụng.
...
Hệ thống hỗ trợ hạn mức, quota, budget hoặc policy theo tổ chức, developer, Agent hoặc phạm vi sử dụng được cấu hình.

### FR06 -- Kiểm soát chi phí trước thực thi

Hệ thống ước lượng hoặc xác định chi phí dự kiến và chỉ chuyển tiếp request khi ngân sách/chính sách cho phép.
...
Hệ thống lưu lịch sử request, trạng thái xử lý, provider, model, usage, cost, transaction và trace để phục vụ kiểm tra, báo cáo và đối soát.

### FR10 -- Trả lỗi khi vượt giới hạn

Hệ thống trả lỗi rõ ràng khi request bị từ chối do sai khóa, Agent không hợp lệ, vượt quota, vượt budget hoặc vi phạm policy.
...
Mỗi request cần có đủ thông tin để xác định tổ chức, Agent, API key, provider, model, chi phí, trạng thái và thời điểm xử lý.
...
| UC03 -- Quản lý ngân sách tổ chức/team/developer | FR01, FR05, FR09, FR10 | NFR01, NFR02, NFR03 |
...
| UC08 -- Quản lý lập trình viên/thành viên tổ chức | FR01, FR02, FR05, FR09, FR10 | NFR01, NFR02, NFR03 |
...
- Với hình ảnh, biểu đồ do tác giả tự xây dựng, caption nên ghi rõ: **Nguồn: Tác giả xây dựng**.
